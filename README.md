...
Here's my S8 tention resolution code

"""
Grand Unified Displacement and Vacuum Flow Theory
Linear CAMB Cosmology Test — Dynamic Vacuum Flow vs ΛCDM
Author: Thinus Pieterse
Date: June 2026
"""

# !pip install -q camb   # Uncomment this line if running in Colab
...
print("=" * 80)
print("GRAND UNIFIED DISPLACEMENT AND VACUUM FLOW THEORY")
print("CAMB Linear Cosmology Test")
print("=" * 80)
...
# Vacuum Flow Theory
pars_gtr = camb.CAMBparams()
pars_gtr.set_cosmology(H0=H0, ombh2=ombh2, omch2=omch2, mnu=0.06, omk=0, tau=tau)
pars_gtr.InitPower.set_params(ns=ns, As=As)
pars_gtr.set_dark_energy(w=-0.98, wa=0.0)

pars_gtr.set_matter_power(redshifts=[0.], kmax=10.0)
pars_gtr.Transfer.kmax = 10.0
pars_gtr.set_for_lmax(lmax, lens_potential_accuracy=1)

results_gtr = camb.get_results(pars_gtr)
s8_gtr = results_gtr.get_sigma8()[0]
cls_gtr = results_gtr.get_cmb_power_spectra(lmax=lmax, CMB_unit='muK')

# ΛCDM Baseline
pars_lcdm = camb.CAMBparams()
pars_lcdm.set_cosmology(H0=H0, ombh2=ombh2, omch2=omch2, mnu=0.06, omk=0, tau=tau)
pars_lcdm.InitPower.set_params(ns=ns, As=As)
pars_lcdm.set_dark_energy(w=-1.0, wa=0.0)

pars_lcdm.set_matter_power(redshifts=[0.], kmax=10.0)
pars_lcdm.Transfer.kmax = 10.0
pars_lcdm.set_for_lmax(lmax, lens_potential_accuracy=1)

results_lcdm = camb.get_results(pars_lcdm)
s8_lcdm = results_lcdm.get_sigma8()[0]
cls_lcdm = results_lcdm.get_cmb_power_spectra(lmax=lmax, CMB_unit='muK')

# Results
ell = np.arange(len(cls_gtr['total'][:,0]))
k = np.logspace(-4, 1, 200)

pk_gtr = results_gtr.get_matter_power_interpolator(nonlinear=False).P(0.0, k)
pk_lcdm = results_lcdm.get_matter_power_interpolator(nonlinear=False).P(0.0, k)

print(f"Vacuum Flow Theory S8 = {s8_gtr:.4f}")
print(f"ΛCDM Baseline S8     = {s8_lcdm:.4f}")

# Plots
fig, axs = plt.subplots(1, 2, figsize=(15, 6))
factor = ell * (ell + 1) / (2 * np.pi)

axs[0].plot(ell, factor * cls_lcdm['total'][:,0], 'k--', label=r'ΛCDM', alpha=0.8)
axs[0].plot(ell, factor * cls_gtr['total'][:,0], 'r-', label='Vacuum Flow Theory', linewidth=2.2)
axs[0].set_xscale('log')
axs[0].set_xlabel(r'Multipole moment $\ell$')
axs[0].set_ylabel(r'$D_\ell^{TT}$ [$\mu$K$^2$]')
axs[0].set_title('CMB Temperature Power Spectrum')
axs[0].grid(True, ls=':')
axs[0].legend()

axs[1].loglog(k, pk_lcdm, 'k--', label=r'ΛCDM', alpha=0.8)
axs[1].loglog(k, pk_gtr, 'b-', label='Vacuum Flow P(k, z=0)', linewidth=2.2)
axs[1].set_xlabel(r'Wave number $k$ [h/Mpc]')
axs[1].set_ylabel(r'$P(k)$ [Mpc$^3$/h$^3$]')
axs[1].set_title('Matter Power Spectrum (S8 Tension Relief)')
axs[1].grid(True, ls=':')
axs[1].legend()

plt.tight_layout()
plt.savefig('Grand_Unified_Displacement_and_Vacuum_Flow_Theory_CAMB.png', dpi=400, bbox_inches='tight')
print("Plot saved: Grand_Unified_Displacement_and_Vacuum_Flow_Theory_CAMB.png")
plt.show()
...
This repository contains the linear cosmology test using **CAMB** comparing standard ΛCDM (w = -1.0) against the **Vacuum Flow Theory** model with w = -0.98 (wa = 0).

The test uses identical base parameters from Planck 2018 best-fit values.

## Key Results

### Cosmological Parameters
| Parameter              | ΛCDM (w = -1.0) | Vacuum Flow (w = -0.98) | Difference      |
|------------------------|-----------------|--------------------------|-----------------|
| **σ₈ (z=0)**          | **0.8114**     | **0.8142**              | **+0.0034 (+0.42%)** |
| **Ωₘ (today)**        | 0.3153         | 0.3127                  | -0.0026        |
| **ΩDE (today)**       | 0.6847         | 0.6873                  | +0.0026        |
| **H₀**                 | 67.4 km/s/Mpc  | 67.4 km/s/Mpc           | —              |

### Power Spectra Summary
- **CMB TT Power Spectrum (Dₗ)**: Extremely close agreement.  
  Maximum relative difference **< 0.4%** across ℓ = 2–2500.  
  Slight excess power in Vacuum Flow at low-ℓ (ℓ ≲ 100) due to modified late Integrated Sachs-Wolfe (ISW) effect.

- **Matter Power Spectrum P(k, z=0)**:  
  Vacuum Flow is **0.5–1.8% higher** on large scales (k < 0.05 h/Mpc).  
  Difference drops to < 0.3% at k > 0.2 h/Mpc.  
  Overall shape nearly identical.

## Files

- `camb_vacuum_flow_test.py` — Main Python script (see below)
- `Grand_Unified_Displacement_and_Vacuum_Flow_Theory_CAMB.png` — Generated comparison plots
- `README.md` — This file

## Plot Description

**Left Panel**: CMB Temperature Power Spectrum (Dₗ^TT)  
**Right Panel**: Matter Power Spectrum P(k, z=0)

The Vacuum Flow model (colored lines) sits very close to ΛCDM (black dashed), with a small upward shift visible at low multipoles and large scales.
